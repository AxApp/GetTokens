...
- 进展补充：2026-06-20 追加完成 `SessionManagementView.tsx` token/shadow cleanup、`CardSections.tsx` typography cleanup、`ThemePresetPicker.tsx` quiet shell 与 `RateLimitRulesSection.tsx` typography cleanup；这四个文件不再作为当前运行态旧样式高信号入口。下一批运行态旧样式高信号入口更新为 `AccountDetailSections.tsx`、`quotaColor.ts`、`UnifiedAccountDetailModal.tsx`、`AssetWorkbenchShell.tsx` 和 `StatusSnippetPanel.tsx` 等小切片。
