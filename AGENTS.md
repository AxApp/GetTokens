...
8. 若用户以“整理”作为收尾指令，且本轮存在稳定可复用模式，不需要额外追问是否沉淀，直接进入 `skills` / `AGENTS` / docs / memory 的整理流程。
...
12. 每轮修复的验收文档必须单列“真实 dev App 手点验收”结果，记录 dev App 启动方式、正式版未触碰确认、手点路径、截图路径和是否发现 native/runtime 问题。
