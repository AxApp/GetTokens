...
5. `其他` 只保留五个业务条件：`有额度且有余额`、`无额度但有余额`、`无额度且无余额`、`今日已使用`、`今日未使用`。
6. `今日已使用 / 今日未使用` 以账号 usage summary 的今日请求数为准，不做前端假状态。
7. 旧 `hasLongestQuota / hasBalance` 筛选存储只作为迁移输入；新筛选状态使用 `quotaAndBalance / noQuotaAndBalance / noQuotaNoBalance / hasUsageToday / noUsageToday`。
8. 2026-05-23 无头浏览器验收截图：`screenshots/20260523/accounts/20260523-accounts-filter-and-after-v01.png`。
