...
3. Given 用户已打开账号详情页，When 该账号匹配到可展示的本地 CLI 应用目标，Then 详情页左侧 SectionNav 下方展示 `应用到 Codex` / `应用到 Claude Code` 动作，disabled 状态和原因必须复用账号卡同一套 mapping。
4. Given 账号详情页同时存在关闭、本地 CLI 应用和保存类动作，When 用户扫描详情页，Then 关闭入口必须以内容区右上角低权重图标按钮呈现，本地 CLI 应用入口固定在左侧导航下方，footer 只保留保存类主操作与状态，避免关闭、写入和保存混在一起。
