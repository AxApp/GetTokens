...
历史上 sidecar 账号轮动能力分散在 `round-robin`、`fill-first`、priority、cooldown、manual-disabled guard、rate-limit guard、请求级 `RoutePolicy`、retry/fallback 和 Codex WebSocket pinned auth 热切等路径中。

2026-05-30 后，CLIProxyAPI 旧公共 `RoutePolicy` 兼容层与 `X-GetTokens-Route-*` / executor metadata allow/deny/order/fallback 请求级注入入口已删除。当前 GetTokens 路由系统由 sidecar 内的 `internal/gettokensrouting` 独立维护，channel routing、account route guard 和 session affinity 都直接进入 GetTokens routing registry。

这些能力的核心热路径已经归一到 `gettokensrouting`，但仍要长期避免三个风险：

1. 自定义端点路由即将上线，如果继续把规则散落到 handler、selector、scheduler 或 executor，后续维护成本会快速上升。
2. `session-affinity`、WebSocket pinned auth 等生命周期逻辑必须在可控边界重新进入同一个 routing registry，不能形成第二套路由系统。
3. GetTokens 已断开与 CLIProxyAPI 上游旧路由系统的合并式维护；后续只能把上游提交作为参考输入，不能为了兼容旧合约把业务判断堆回上游核心文件。

本 space 用于规划并分阶段落地 `Account Routing Engine`：把“路由上下文归一化 -> 账号候选 -> 策略改写 -> 账号选择 -> 执行器”收敛为一个可解释、可重试、可观测、低冲突的路由决策入口。

本期同时承担一次性清理任务：把过去已经实现但散落在 `RoutePolicy`、`AccountRouteGuardStore`、rate-limit policy、session affinity selector、WebSocket pinned auth 和启动 hook 中的 GetTokens 路由逻辑重新归档到统一边界内。2026-05-30 已完成旧公共 `RoutePolicy` 和旧请求级注入入口删除；后续路由能力只在 GetTokens sidecar 独立维护。
...
- sidecar / CLIProxyAPI fork：
  ...
  - 将 endpoint route policy、guard policy、channel routing、sticky policy 纳入 pipeline。
  - 修复或确认 `InstallRoutingPolicies`、usage attribution、rate-limit hook 的生产启动安装点。
  - 将 rate-limit 双路径收敛为统一 guard source 输出，避免重复 deny。
  - 将 session affinity 适配到 engine pipeline，避免绕过 scheduler fast path。
...
- 不要求一次性重写所有 scheduler / selector 代码；先建立稳定 seam，再逐步迁移。
...
15. Given `InstallRoutingPolicies`、usage attribution、rate-limit hook 过去只在定义处可见，When 清理完成，Then 生产启动链路有明确安装点和自动化测试覆盖。
16. Given 旧 `RoutePolicy` header/metadata 调试入口曾被 Codex/Claude 路由探测使用，When 清理完成，Then 这些入口已删除，路由探测改走 GetTokens channel routing、guard source 和 routing registry 测试面。
...
- 2026-05-26 补充：Codex 账号列表的旧 `session-affinity` / `websocket-pin` / `route-order-header` 曾作为 `兼容层提示` 的总数与说明呈现，不写入新的 `ChannelRoutingConfig`，也不展开三条明细。2026-05-30 旧 request-level route-order 注入面已删除，后续诊断以 route ledger、guard source 和 focused tests 为准。
...
旧实施入口已完成本轮清理。后续开发从 `internal/gettokensrouting` 和 channel routing / guard source / route ledger 的独立维护边界继续，不再回到 CLIProxyAPI 旧 RoutePolicy 兼容层。
