...
1. 账号池主列表筛选采用分组叠加：组内多选为 OR，组间为 AND。
2. 筛选面板只保留三组：`套餐与来源`、`状态`、`其他`。
3. `套餐与来源` 继续承接动态套餐键与来源选择；套餐来自账号 `planType` 与 quota `planType`，来源只区分 `AUTH FILE` 与 `API KEY`。
4. `状态` 继续保留 `异常 / 已禁用 / 可请求`，禁用与异常不得混为同一个状态。
5. `状态` 组中的 `异常` 需要支持按请求状态码细分，不再只有一个粗粒度标签。
6. 请求状态码筛选默认按当前账号数据中实际出现的错误码动态生成，例如 `401 / 402 / 403 / 429`。
7. 请求状态码的证据来源优先级：
   - quota / billing runtime 的 `degradedReason`、上游失败描述
   - 账号 `statusMessage`
   - `requestability.evidence`
8. 请求状态码作为 `状态` 组内的独立子维度：
   - 基础状态仍保留 `异常 / 已禁用 / 可请求`
   - 请求状态码子维度默认不过滤
   - 当用户选择 `401` 或 `402` 时，应只命中带对应错误证据的账号
9. 若错误没有明确 HTTP 状态码，仍由基础 `异常` 兜底，不强行伪造 `0` 或 `unknown` 筛选项。
10. `其他` 下一期计划改为三个独立业务维度：`额度`、`余额`、`今日使用`。
11. 每个维度只提供成对筛选项，不再提供预组合条件：
   - `额度`：`有额度 / 无额度`
   - `余额`：`有余额 / 无余额`
   - `今日使用`：`今日有使用 / 今日无使用`
12. 每个维度内部采用互斥选择并保留“全量”默认态：
   - 默认 `有 / 无` 同时选中，表示不过滤该维度
   - 点掉其中一个后，仅按剩余选项过滤
   - 不允许同一维度两个选项同时关闭，避免出现不可恢复的空配置
13. 三个维度彼此独立，可组合形成 `有额度 + 无余额`、`无额度 + 今日有使用` 等真实业务筛法，不再把“额度”和“余额”绑死成单个组合条件。
14. `今日有使用 / 今日无使用` 以账号 usage summary 的今日请求数为准，不做前端假状态。
15. 旧 `hasLongestQuota / hasBalance` 及 `quotaAndBalance / noQuotaAndBalance / noQuotaNoBalance` 存储只作为迁移输入；下一期新筛选状态计划在前端模型中拆为独立的 `quota / balance / usageToday` 维度。
16. 2026-05-23 无头浏览器验收截图：`screenshots/20260523/accounts/20260523-accounts-filter-and-after-v01.png`。

### 2026-06-09 筛选重构证据

1. 问题来源：用户在账号池筛选面板中直接指出 `今日已使用` 所在整组“不实用”，希望改成 `有额度 / 无额度 / 有余额 / 无余额 / 今日有使用 / 今日无使用` 六个业务筛项。
2. 当前代码位置：
...
3. 当前现象：资源筛选把“额度”和“余额”耦合成三种预组合条件，导致用户无法直接筛出“有额度但无余额”“无额度但无余额之外的其他组合”等单维状态。
4. 预期改动：
   - 资源筛选改为三个独立维度：额度、余额、今日使用
   - 状态筛选中的异常支持按请求状态码细分，至少覆盖当前数据里实际出现的 `401 / 402 / ...`
   - 工具栏交互允许独立组合三个维度，而不是继续使用预组合标签
   - 筛选摘要与空态判断同步使用新维度
5. 验收方式：
