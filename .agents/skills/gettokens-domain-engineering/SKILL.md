...
- Codex HTTP `/v1/responses` 413 diagnostics: distinguish local relay failures from upstream request-body byte limits. ChatGPT Codex Responses can reject JSON bodies above about 10 MiB; when sidecar logs show `request body too large: limit is 10485760 bytes`, treat it as an upstream/transport payload issue, not a frontend restriction. The sidecar hot path may zstd-compress large translated Codex Responses bodies before upstream; small requests must remain uncompressed. If a 413 is followed by 401 `token invalidated` and then 503 `auth_unavailable`, report the chain as payload-too-large plus credential invalidation/route-guard exhaustion instead of blaming a single UI change.
