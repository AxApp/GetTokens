...
test('ProxyPoolFeature uses the current workspace table layout', async () => {
  ...
  assert.match(source, /aria-label="新增代理"[\s\S]*className="inline-flex h-10 w-10 items-center justify-center rounded border border-\[var\(--gt-border-strong\)\] bg-\[var\(--text-primary\)\] text-\[var\(--gt-surface-canvas\)\][\s\S]*<Plus className="h-5 w-5" strokeWidth=\{2\.5\} \/>/);
