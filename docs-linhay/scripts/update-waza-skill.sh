...
usage() {
  cat <<'EOF'
Usage:
  update-waza-skill.sh [--check|--update]

Modes:
  --check   Compare the pinned commit with the remote ref. This is the default.
  --update  Replace installed Waza skill directories and update the lock file.

Environment:
  LOCK_PATH  Override the Waza lock file path.
EOF
}
...
LOCK_PATH="${LOCK_PATH:-$REPO_ROOT/.agents/skills/waza.lock.json}"
...
PINNED_VERSION="$(read_lock_field source.version)"
...
if [[ "$MODE" == "check" ]]; then
  if [[ "$PINNED_COMMIT" == "$REMOTE_COMMIT" ]]; then
    echo "Waza is up to date."
    exit 0
  fi

  echo "Waza update available. Run with --update to install it."
  exit 2
fi
...
for item in data.get("sharedPaths", []):
    source = checkout_root / item["sourcePath"]
    destination = repo_root / item["localPath"]
    if not source.is_dir():
        raise SystemExit(f"Missing shared path in checkout: {source}")
    if destination.exists():
        shutil.rmtree(destination)
    destination.parent.mkdir(parents=True, exist_ok=True)
    shutil.copytree(source, destination)
...
data["source"]["version"] = (checkout_root / "VERSION").read_text(encoding="utf-8").strip()
data["installedAt"] = date.today().isoformat()
lock_path.write_text(json.dumps(data, indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
PY

echo "Updated Waza to $REMOTE_COMMIT."
