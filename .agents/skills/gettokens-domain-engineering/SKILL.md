...
- **Rules**:
  ...
  - Vendor presets with `quotaCurlTemplate` or `billingCurlTemplate` (for example DeepSeek) are balance-aware account assets and should be created as `codex-api-key`, not `openai-compatible`. Reuse the existing quota/billing refresh path instead of adding provider-card UI special cases or extending `openai-compatible` for balance display unless a future requirement explicitly asks for that model change.
...
- **Codex Live Sessions**: For `#frame=codex&workspace=live-sessions`, treat the feature as runtime observability, not local session-file management. Use this when surfacing in-flight request/session state from CLIProxyAPI.
  ...
  - On structural snapshot merge for live sessions: when `source` and `sidecarReady` are the same between polls, sessions omitted by the next snapshot must be retained in the feed instead of removed. Only source/ready state transitions, user-initiated clear, or explicit filter changes should shorten the list. This prevents the "flash-disappear" bug where a single short sidecar snapshot temporarily drops a session from the navigation feed.
...
- **Account Route Guard & WebSocket Hot Switch**:
  ...
  - Auth-file OAuth usage refresh that receives upstream non-2xx, especially ChatGPT `401 token_invalidated`, must write stale/degraded quota runtime with the upstream message in `degradedReason`. Do not hide it as a silent cache fallback; account cards and detail quota sections must surface stale/degraded quota runtime reasons.
