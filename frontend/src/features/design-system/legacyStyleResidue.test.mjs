...
const legacyRuntimeStylePattern =
  /btn-swiss|input-swiss|select-swiss|card-swiss|shadow-\[|shadow-(?:lg|xl|2xl)|drop-shadow|border-2|bg-\[var\(--bg-(main|surface)\)\]|--color-(?:chart|status)-|--font-size-|font-(?:medium|bold|extrabold|black)|\buppercase\b|tracking-\[|backdrop-blur|\btransition(?![-\[])|transition-all|transition-transform|transition-opacity|hover:opacity|active:opacity|group-hover:opacity|active:scale|animate-pulse|animate-spin|grayscale|text-\[(?:9|10|11)px\]|!?text-(?:xs|sm|base|lg|xl|2xl|3xl)\b|opacity-(?:10|30)(?!\d)|usage-desk-(?:curve|area|point)|rangeAnimationVersion|curveMotion|detailTransitionActive|codex-live-(?:chart-enter|point-pulse)|codex-success-hud/;
...
test('runtime inline styles stay limited to approved dynamic rendering boundaries', async () => {
  const allowedInlineStyleBlocks = [
    ...
    { path: 'features/accounts/components/CardSections.tsx', pattern: /fillPercent|row\.fillPercent|activityPercent|marginBottom/ },
    { path: 'features/codex/CodexFeature.tsx', pattern: /resolveAutoWidthInputCh/ },
