...
test('session analysis opens from header through scope selector and detail modal', async () => {
  ...
  assert.doesNotMatch(viewSource, /--text-on-accent|--bg-(main|surface|subtle|warning)/, 'session-management controls must not use legacy inverse background text tokens');
...
test('session management analysis and raw json use the quiet detail shell', async () => {
  ...
  assert.match(resultBlock, /text-\[length:var\(--gt-font-size-base\)\]/, 'analysis word cloud must use gt typography tokens');
  assert.doesNotMatch(resultBlock, /!?text-(?:xs|sm|base|lg|xl|2xl|3xl)\b/, 'analysis word cloud must not use naked Tailwind typography sizes');
