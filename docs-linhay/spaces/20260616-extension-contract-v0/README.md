...
- 2026-06-19 追加 staged temp apply UI：Extension Registry 显示 `Staged Temp Apply` 测试面板，prepare/apply 只允许显式 `/tmp/gettokens-extension-codex-config-staged-preview.toml` 目标；浏览器 preview 或无 Wails runtime 时 blocked，真实 `~/.codex/config.toml` local apply 仍需后续明确授权。
- 2026-06-20 UI 迁移追加：Extension Registry 页面按账号池/代理池/会话管理形成的新工作台风格收敛，主列表、toolbar chips、notice、右侧 dry-run / staged apply / roots / diagnostics / selected extension 区统一到 `--gt-*` quiet shell；新增源码门禁锁住 `data-gettokens-extension-registry-list-header`、`data-gettokens-extension-registry-aside`、`data-gettokens-extension-registry-selected`，并禁止回退到旧粗边框、`bg-main/bg-surface` 壳层和 heavy uppercase。
- 2026-07-07 浏览器批注收紧：Extension Registry toolbar 移除 generated/message 调试文案与长说明 notice，搜索框改为随父容器占满宽度；源码测试和 preview gate 更新为结构/边界校验，继续锁住 local enable-state only、read-only/staged apply 边界与 no marketplace/no Codex config mutation。
- 第十轮入口切片目标与证据门禁：
...
- OmniRoute Productization staged temp apply UI：前端 registry 页面已消费上述 Wails/root DTO，展示 confirmation token、diff preview、apply result、rollback label；preview gate 锁住 `stagedApplyTestSurface`、`stagedApplyActions`、`stagedApplyRuntimeBoundary`。
