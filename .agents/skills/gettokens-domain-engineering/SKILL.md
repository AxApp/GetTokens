...
- **Risky Upstream Rule**: Upstream PRs or commits are inputs, not automatically accepted product behavior. If an upstream change can corrupt or bypass GetTokens sidecar autonomy, reimplement the reasonable part inside the maintained sidecar boundary, add narrow fork-side regression tests, push `origin/gettokens/sidecar`, rebuild the local sidecar, and only then merge the parent GetTokens changes back to the main branch.
