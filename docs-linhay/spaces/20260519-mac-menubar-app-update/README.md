...
- 状态：implemented
- 最近更新：2026-05-19
- 最近变更：扩展为 macOS 状态栏驻留与应用生命周期设置；明确关闭窗口保留服务时必须提供状态栏恢复 / 退出入口，首期状态栏视图采用原生菜单。
...
- 验证：已通过菜单模型测试、应用生命周期 Go 测试、`go test ./...`、`npm --prefix frontend run test:unit`、`npm --prefix frontend run typecheck`、`./scripts/wails-cli.sh build` 和 System Events 桌面菜单读取验证。
