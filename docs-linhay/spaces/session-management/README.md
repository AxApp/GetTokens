...
- 追加收敛设计系统中的 `SessionPluginConsolePanel`：插件列表、scope、执行状态、会话队列和输出面板统一使用轻描边组件壳层，移除旧 thick border / Swiss button / heavy uppercase。
...
- 插件控制台固定 `data-session-plugin-console-*` 结构，新增源码测试确保不回退到旧 `btn-swiss`、粗边框和大写重字重。
