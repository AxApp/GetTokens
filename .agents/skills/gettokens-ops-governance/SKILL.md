...
- **Management API Smoke Auth**: For direct curl smoke against sidecar management routes, authenticate with the App/sidecar local management key (`Authorization: Bearer gettokens-local-management-key`). Do not use the top-level relay `api-keys`; those authenticate proxy clients and will return 401 on management routes.
