...
test('ProjectCandidatePoolRulesPanel stays flat inside the project config modal', async () => {
  ...
  assert.match(source, /import \{ Button, Checkbox, Select, Tag \} from 'antd'/);
  assert.match(source, /<Select[\s\S]*className="w-full"/);
...
test('ChannelRoutingWorkbench uses left conditions and right diagnostic result layout', async () => {
  ...
  assert.ok((source.match(/<Select/g) || []).length >= 2);
  assert.match(source, /className="min-w-0 font-mono"/);
  assert.match(source, /<Select[\s\S]*size="small"/);
  assert.match(source, /options=\{options\.map\(\(option\) => \(\{ value: option\.value, label: option\.label \}\)\)\}/);
