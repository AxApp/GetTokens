...
- 第七轮开始接真实数据：页面新增“导入环境代理”，可把当前进程里的 `https_proxy / http_proxy / all_proxy` 导入为本地节点；`复测` 相关动作也已改成真实代理探测，不再只做本地模拟。
...
- 本轮生成链已确认修正方向正确：`FetchProxySubscription / ListEnvironmentProxyEntries / ProbeProxyNode` 都已进入 `frontend/wailsjs/go/main/App` 生成绑定。
