...
- 非原生更新器场景在 App 启动后立即检测一次，之后每 6 小时定时检测一次
...
1. Sidebar 底部版本区固定保留在左侧栏底部，默认只显示 `VERSION <release-label>`。
2. 真实 App 启动后会异步做一次非原生更新检查；之后每 6 小时自动检测一次。只有后端检测到新版本并发出 `updater:available` 后，底部版本区才出现升级提示按钮。
3. 设置页手动“检查更新”若拿到新版本，也会把同一个 `availableRelease` 写回全局状态，因此 Sidebar 底部同步出现升级提示。
4. 普通浏览器预览和无 Wails runtime 场景不显示升级提示；只有 `?preview=sidebar-update` 用于稳定复现该状态。
5. macOS Sparkle / 原生更新器场景以设置页、App 菜单或状态栏菜单触发原生 UI 为主，不依赖 Sidebar 自动弹出自定义升级提示。
...
- 非原生更新器场景下，后台更新检测启动即执行一次，并以 6 小时间隔继续轮询
