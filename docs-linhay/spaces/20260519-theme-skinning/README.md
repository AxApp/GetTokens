...
- 进展补充：2026-06-20 追加完成 `SessionManagementView.tsx` token/shadow cleanup；该文件不再作为当前运行态旧样式高信号入口。下一批运行态旧样式高信号入口更新为 `CardSections.tsx`、`ThemePresetPicker.tsx`、`RateLimitRulesSection.tsx`、`AccountDetailSections.tsx` 和 `quotaColor.ts` 等小切片。
