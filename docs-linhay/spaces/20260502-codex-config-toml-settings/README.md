...
一期范围已扩展为 Codex `[features]` + `[notice]` 下的 bool 开关快捷配置。现有能力仍偏向“接入 GetTokens relay 的一次性应用”，用户缺少一个更直接的入口：读取当前本机 `config.toml`，展示可配置的 bool feature / notice，保存前预览差异，并在不破坏用户手写配置、注释、MCP、agents 等扩展段落的前提下落盘。

## 目标
1. 支持对 Codex 本地 `config.toml` 中 `[features]` 与 `[notice]` bool 开关做快捷配置。
2. 将读取、预览、保存收敛到后端统一能力，避免前端拼 TOML 或整文件覆盖。
3. 保存时只修改 `[features]` + `[notice]` 下用户本次操作的 bool 键，保留未知字段、注释、换行风格和无关 section。
4. 给用户提供保存前的风险感知：目标路径、将修改的键、备份路径或恢复方式、错误原因。

## 范围
本期只覆盖 `~/.codex/config.toml` 的 `[features]` bool 配置，以及 `[notice]` 下的 bool 提示开关：
...
- 读取 `[notice]` 下已存在的 bool 键。
- 展示 Codex 当前源码支持的 notice bool key。
- 支持开关并保存用户明确修改的 notice bool key。
...
3. 不修改 `model`、`model_provider`、`model_reasoning_effort`、provider、MCP server、agents role、profiles、projects trust_level 等非 feature 配置。
...
6. 不管理 `notice.model_migrations`、`notice.external_config_migration_prompts`、`mcp_servers`、`skills.config`、`projects."<path>".trust_level` 这类复合或结构化配置。
...
2. 写入范围限定在 `CODEX_HOME/config.toml` 的 `[features]` + `[notice]` bool key；保存只提交用户显式修改值，不 materialize 所有默认值。
...
4. `Feature 配置` 页面使用全宽 `Codex Features` 表格工作区，`Codex Notices` 子区块纳入 `[notice]` 提示开关：每个 item 独立一行，开关直接显示当前 ON/OFF 与默认 `true/false`。
...
7. `hide_rate_limit_model_nudge` 等 `notice` bool 已纳入页面；`notice.model_migrations`、`mcp_servers`、`skills`、project trust level 仍保留在其他专用入口或后端配置路径。
8. Gemini 用量入口暂不暴露，后续 Gemini 能力成型后再单独纳入导航。
9. 旧 `#frame=session-management`、`#frame=vendor-status`、`#frame=usage-desk` 路由和本地存储值保留兼容迁移，统一进入 `Codex` 对应二级项。
10. 实现截图：[`20260502-codex-config-codex-menu-after-v01.png`](screenshots/20260502/codex-config/20260502-codex-config-codex-menu-after-v01.png)。
