...
- Account-page automatic runtime sync must use adaptive intervals for large pools. A 30s whole-pool loop is acceptable only for small pools; once the pool reaches hundreds/thousands of accounts, slow the automatic loop and rely on explicit header/group/card refresh for user-intent freshness. Avoid an immediate duplicate whole-pool sync right after `loadAccounts()` already launched supplemental runtime refresh.
...
- Local caches must be write-if-changed, bounded, and disabled in Wails runtime when they duplicate backend cache without clear benefit. Do not let `updatedAt` or other timestamp-only fields force full WebView `localStorage` rewrites for unchanged account/quota payloads; this expands WebKit SQLite WAL files and can keep WebContent memory high even when the actual cache key payload is only a few MB.
