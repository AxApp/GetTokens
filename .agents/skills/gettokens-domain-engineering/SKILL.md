...
## Session Distillation: Account inventory, detail, mutation, and runtime resources
- Account inventory exposed to WebView must be a secret-free summary. API keys, cookies, auth JSON, proxy credentials, custom headers, cURL variables, and model-fetch credentials belong only in a scoped detail response and must not enter list cache, account events, or summary mutation patches.
- Account detail is loaded on demand by stable `acct_*` key. Cache/dedupe may use `account_key + revision`, but closing or switching the modal must not turn detail credentials into a cross-account list cache.
- Account mutations return the authoritative post-write account record. Frontend must patch the matching account and selected detail from that response instead of issuing a save-after-full-list reload.
- Mutable account configuration uses account-store `revision` plus `expected_revision`. A stale write returns `409 account_revision_conflict`, performs no DB write and no runtime apply. Frontend recovery is explicit conflict notice plus latest-detail reload; do not automatically merge or retry user edits.
- Inventory uses a stable `inventory_revision` over summary-relevant asset fields. If unchanged, do not replace the frontend collection. Account change events carry a monotonic event id and secret-free summary; direct patch is allowed only for an in-order event, while gaps or invalidation trigger a low-frequency inventory reconcile.
- Quota, rate-limit, usage attribution, and route decisions have independent freshness/decision clocks. Preserve their own evaluated/generated/reset timestamps and stale/degraded evidence; do not invent one global version that implies a cross-resource atomic snapshot.
- Usage attribution account ownership is sidecar-only. Runtime executors/reporters propagate the selected internal auth's stable `AccountKey`; Wails/frontend must not rescan full accounts, read local historical identity files, or infer ownership from provider/display fields.
- Root/public `ListAccounts` compatibility may return the safe inventory summary only. Full account records may remain for backend-only credential/routing commands while they are still needed, but must not be re-exported to WebView or reused as a general read model.
...
## Session Distillation: Generation-owned account runtime facts
- `accounts-v2.sqlite` 只保存资产、credential generation 和 token revision；route guard、quota snapshot 等可丢弃运行事实写入 sidecar-owned `runtime-v1.sqlite`，不得回写 channel routing JSON。
- 每条可持久 runtime fact 必须携带 `account_key + credential_generation + runtime_auth_id + observed_at + expires_at/retention`。恢复、路由消费和 doctor 只接受当前资产 generation；旧代即使尚未 GC 也不得生效。
- credential generation 只在用户替换凭证或 OAuth 重新登录时递增。标题、priority、disabled、metadata、models 等资产编辑不得误推进 generation；同代 token refresh 只推进 token revision。
- OAuth callback 对既有账号的提交顺序固定为：验证 callback/session -> SQLite `CommitOAuthCredential` CAS -> 编译并交换 runtime snapshot -> 才允许 scheduler/hook/guard 看见新代。旧代 refresh 的迟到结果不得写内存或任何 runtime fact。
- channel routing `runtimeStates` 属于已废弃历史机制。sidecar/Wails 不反序列化、不 hydrate、不参与 explain/probe/route、不写回；策略文件下一次保存可以直接移除该字段。
- Runtime hot path 先同步更新有界内存索引，再持久化诊断事实。runtime DB 写失败不得让旧 generation 获得权限，也不得回退到 auth-dir、FileTokenStore、v1 account DB 或 provider identity 扩散。
