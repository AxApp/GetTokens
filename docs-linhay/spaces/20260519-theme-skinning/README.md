...
- 进展补充：2026-06-20 追加完成 `SessionManagementView.tsx` token/shadow cleanup 与 `CardSections.tsx` typography cleanup；这两个文件不再作为当前运行态旧样式高信号入口。下一批运行态旧样式高信号入口更新为 `ThemePresetPicker.tsx`、`RateLimitRulesSection.tsx`、`AccountDetailSections.tsx`、`quotaColor.ts` 和 `UnifiedAccountDetailModal.tsx` 等小切片。
