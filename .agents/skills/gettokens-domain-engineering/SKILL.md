...
- **Rules**:
  ...
  - Account-detail mutations that change visible account fields (`apiKey`, `baseUrl`, `prefix`, label/name, priority, quota/billing scripts, model mappings, or openai-compatible provider fields) must patch the local account-list cache by stable `account.id` after the backend save succeeds. `ListAccounts()` is the final authoritative refresh, but it must not be the only UI consistency path; if reload is delayed or fails transiently, reopening detail from the account card must still show the saved values. Never patch by recomputing config fingerprints or replacing `acct_*`.
