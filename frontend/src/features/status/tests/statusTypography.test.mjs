...
test('status page uses AntD workbench shell for the redesigned status frame', async () => {
  ...
  assert.match(statusFeatureSource, /statusPageShellClass[\s\S]{0,120}--gt-surface-muted/);
...
test('status panels use the quiet workspace shell', async () => {
  ...
  assert.match(statusPanelSource, /<Card\b/);
  ...
  assert.match(snippetPanelSource, /<Card\b/);
...
test('status local CLI apply panel uses the workbench plan layout', async () => {
  ...
  assert.match(statusPanelSource, /data-status-local-cli-card=\{section\}/);
  assert.match(statusPanelSource, /section="codex-connection"/);
  assert.match(statusPanelSource, /section="codex-routing"/);
  assert.match(statusPanelSource, /section="codex-capabilities"/);
  assert.match(statusPanelSource, /section="codex-preflight"/);
  assert.match(statusPanelSource, /section="claude-connection"/);
  assert.match(statusPanelSource, /section="claude-models"/);
  assert.match(statusPanelSource, /section="claude-runtime"/);
  assert.match(statusPanelSource, /section="claude-preflight"/);
  ...
  assert.equal(zh.status.local_cli_apply_blocked_not_ready, '本地服务还未就绪，暂时不能写入本地 CLI 配置');
  assert.equal(zh.status.local_cli_apply_blocked_missing_key, '需要先选择一个 Relay API Key，才能写入本地 CLI 配置');
  assert.equal(zh.status.local_cli_config_input_title, '配置输入');
  assert.equal(zh.status.local_cli_connection_card_title, '连接与凭证');
  assert.equal(zh.status.local_cli_codex_routing_card_title, 'Provider 与 Model');
  assert.equal(zh.status.local_cli_capability_card_title, '能力开关');
  assert.equal(zh.status.local_cli_claude_model_card_title, 'Model 映射');
  assert.equal(zh.status.local_cli_claude_runtime_card_title, '运行参数');
  ...
  assert.equal(en.status.local_cli_apply_blocked_not_ready, 'Local service is not ready yet, so the local CLI config cannot be written.');
  assert.equal(en.status.local_cli_apply_blocked_missing_key, 'Select a Relay API key before writing the local CLI config.');
  assert.equal(en.status.local_cli_connection_card_title, 'Connection and credential');
  assert.equal(en.status.local_cli_codex_routing_card_title, 'Provider and model');
  assert.equal(en.status.local_cli_capability_card_title, 'Capabilities');
  assert.equal(en.status.local_cli_claude_model_card_title, 'Model mapping');
  assert.equal(en.status.local_cli_claude_runtime_card_title, 'Runtime parameters');
