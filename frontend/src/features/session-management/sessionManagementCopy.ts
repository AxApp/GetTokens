...
export function createSessionManagementCopy(
  ...
  return {
    ...
    analysisOpen: isEnglish ? 'Analyze' : '分析',
    analysisSelectorTitle: isEnglish ? 'Choose Analysis Scope' : '选择分析范围',
    analysisSelectorHint: isEnglish ? 'Pick all sessions, a project, recent visible sessions, or one session.' : '选择全部、项目、最近可见会话或单条会话后再运行分析。',
    analysisDetailTitle: isEnglish ? 'Analysis Details' : '分析详情',
    analysisSelectProject: isEnglish ? 'Project Selector' : '项目选择',
    analysisSelectSession: isEnglish ? 'Session Selector' : '会话选择',
    analysisBackToSelection: isEnglish ? 'Back to Selection' : '返回选择',
    analysisTitle: isEnglish ? 'Session Analysis' : '会话分析',
    analysisAll: isEnglish ? 'Analyze All' : '分析全部',
    analysisProject: isEnglish ? 'Analyze Project' : '分析项目',
    ...
    analysisWordCloud: isEnglish ? 'Word Cloud' : '词云',
    analysisCommonPhrases: isEnglish ? 'Common Phrases' : '常用句',
