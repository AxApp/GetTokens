...
const subagentCatalogWarningPanelClass =
  'mb-3 rounded border border-[var(--gt-status-warning)] bg-[color-mix(in_srgb,var(--gt-status-warning)_10%,transparent)] p-3 text-[length:var(--gt-font-size-sm)] text-[var(--gt-status-warning)]';
...
export default function ClaudeCodeSubagentCatalog({
  ...
  return (
    <AssetWorkbenchShell
      title="Subagents"
      subtitle={
        <span className="text-[length:var(--gt-font-size-xs)]">
      ...
      notice={
        stateMessage || errorCount > 0 ? (
          <span className={`flex items-center gap-2 text-[length:var(--gt-font-size-sm)] ${errorCount > 0 ? 'text-[var(--gt-status-warning)]' : 'text-[var(--gt-ink-secondary)]'}`}>
    ...
    >
      {total === 0 && !creatingNew ? (
        <div className="flex flex-col items-center justify-center gap-3 p-12 text-center">
          ...
          <p className="text-[length:var(--gt-font-size-sm)] text-[var(--gt-ink-muted)]">No subagents discovered</p>
            ...
            <div key={agent.path}>
              ...
              >
                ...
                <div className="flex-1 text-left">
                  <span className="text-[length:var(--gt-font-size-sm)] font-normal">{agent.name || <span className="text-[var(--gt-ink-muted)]">unnamed</span>}</span>
                  {agent.scope && (
                    <span className="ml-2 rounded bg-[var(--gt-surface-muted)] px-1.5 py-0.5 text-[length:var(--gt-font-size-xs)] text-[var(--gt-ink-muted)]">{agent.scope}</span>
                  )}
                </div>
                ...
                {agent.isPlugin && (
                  <span className="flex items-center gap-1 rounded bg-[var(--gt-surface-muted)] px-2 py-0.5 text-[length:var(--gt-font-size-xs)] text-[var(--gt-ink-muted)]">
              ...
              {expandedAgents.has(agent.path) && (
                <div className={subagentCatalogPanelClass}>
                  <p className="mb-3 font-mono text-[length:var(--gt-font-size-xs)] text-[var(--gt-ink-muted)]">{agent.path}</p>
                  ...
                  {agent.validationErrors && agent.validationErrors.length > 0 && (
                    <div className="mb-3 space-y-1">
                      {agent.validationErrors.map((err, i) => (
                        <div key={i} className="flex items-center gap-2 text-[length:var(--gt-font-size-xs)] text-[var(--gt-status-warning)]">
                  ...
                  {agent.description && (
                    <p className="mb-3 text-[length:var(--gt-font-size-sm)] text-[var(--gt-ink-secondary)]">{agent.description}</p>
                  )}

                  {agent.knownFields && Object.keys(agent.knownFields).length > 0 && (
                    <div className="mb-3">
                      <div className="mb-1 text-[length:var(--gt-font-size-xs)] font-normal text-[var(--gt-ink-secondary)]">Configuration</div>
                  ...
                  {agent.isPlugin && agent.ignoredFields && agent.ignoredFields.length > 0 && (
                    <div className="mb-3 rounded bg-[color-mix(in_srgb,var(--gt-status-warning)_12%,transparent)] p-2 text-[length:var(--gt-font-size-xs)] text-[var(--gt-status-warning)]">
                  ...
                  {agent.bodyPreview && (
                    <div className="mb-3">
                      <div className="mb-1 text-[length:var(--gt-font-size-xs)] font-normal text-[var(--gt-ink-secondary)]">Body Preview</div>
                      <SnippetPre className="max-h-32 overflow-auto text-[length:var(--gt-font-size-xs)]">{agent.bodyPreview}</SnippetPre>
                    </div>
                  )}
      ...
      {(creatingNew || editingPath) && (
        <div className={subagentCatalogEditorClass} data-claude-subagent-editor="quiet">
          <h3 className="mb-3 flex items-center gap-2 text-[length:var(--gt-font-size-sm)] font-normal">
          ...
          <div className="space-y-3">
            <div>
              <label className="mb-1 block text-[length:var(--gt-font-size-xs)] font-normal text-[var(--gt-ink-secondary)]">Name *</label>
              <Input
                size="small"
                className="font-mono"
                value={draftName ?? ''}
            ...
            <div>
              <label className="mb-1 block text-[length:var(--gt-font-size-xs)] font-normal text-[var(--gt-ink-secondary)]">Description *</label>
              <Input
                size="small"
            ...
            <div>
              <label className="mb-1 block text-[length:var(--gt-font-size-xs)] font-normal text-[var(--gt-ink-secondary)]">Body (Markdown)</label>
              <Input.TextArea
                size="small"
                value={draftBody ?? ''}
...
function AgentFieldRow({ fieldKey, value, ignored }: { fieldKey: string; value: any; ignored?: boolean }) {
  ...
  return (
    <>
      <span className={`font-mono text-[length:var(--gt-font-size-xs)] ${ignored ? 'text-[var(--gt-ink-muted)] line-through' : 'text-[var(--gt-ink-secondary)]'}`}>
        {fieldKey}
      </span>
      <span className={`text-[length:var(--gt-font-size-xs)] ${ignored ? 'text-[var(--gt-ink-muted)]' : 'text-[var(--gt-ink-primary)]'}`}>
