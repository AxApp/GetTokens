...
2. curl 请求能力：
   ...
   - 暂不支持但不影响结构化请求的 curl 参数会被忽略；请求成功时不阻断保存，请求失败时把已忽略参数作为排查提示返回给用户。
...
- 2026-05-19：补齐 unsupported option ignore 与保存前测试回归；`go test ./internal/accounts`、`go test ./internal/wailsapp`、`node --test frontend/src/features/accounts/tests/accountConfig.test.mjs` 通过。
