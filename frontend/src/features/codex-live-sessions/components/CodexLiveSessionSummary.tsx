...
export function SourceBadge({ snapshot }: { snapshot: CodexLiveSessionSnapshot }) {
  ...
  return (
    <span className="inline-flex items-center rounded border border-[var(--gt-border-subtle)] bg-[var(--gt-surface-muted)] px-2.5 py-1.5 font-mono text-[length:var(--gt-font-size-xs)] font-semibold text-[var(--gt-ink-muted)]">
      {label}
    </span>
  );
}
