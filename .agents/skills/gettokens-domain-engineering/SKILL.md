...
- **Account Route Guard & WebSocket Hot Switch**:
  ...
  - Codex API key quota/billing curl HTTP execution should run through the sidecar, either via the generic management `/api-call` bridge or a later sidecar-native quota refresh endpoint. Wails/root may temporarily parse curl and map quota DTOs, but it should not directly open the target provider URL with its own `http.Client`.
