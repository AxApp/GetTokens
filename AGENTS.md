...
25. 每一轮功能修复收尾都必须做真实 dev App 手点验收：使用本仓 dev 构建产物和 `GETTOKENS_APP_PROFILE=dev`，在真实 macOS/Wails 窗口或菜单栏中手动走一遍本轮关键入口，并把截图归档到对应 `space`。自动化测试、浏览器预览、hash 断言和 Wails build 只能作为前置门禁，不能替代真实 dev App 手点验收；若因环境阻塞无法执行，必须在验收文档和最终回复中明确阻塞原因与剩余风险。
...
10. 通用看板产品线每轮都必须先更新对应 `space` 的需求与验收，再补技术细节；技术约束反推需求后，需要回到 `space` 调整需求，再进入设计系统稿、开发、冒烟和用户测试。
11. 每轮修复的验收文档必须单列“真实 dev App 手点验收”结果，记录 dev App 启动方式、正式版未触碰确认、手点路径、截图路径和是否发现 native/runtime 问题。
