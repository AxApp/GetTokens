...
- 完成：`docs-linhay/references/CLIProxyAPI#gettokens/sidecar` 已合并 `upstream/main@c65275fd`（tag `v7.1.28`），生成并推送 merge commit `e2ec7262` 到 `AxApp/CLIProxyAPI#gettokens/sidecar`。
- 冲突处理：`SetTranslatedReasoningEffort` 保留 fork 的“translated payload 无 reasoning 时不清空 context reasoning effort”修复，同时接入 upstream `service_tier`；redisqueue payload 测试同时锁定 `ttft_ms` 和 `service_tier`。
...
- 文档：同步记录已追加到 `docs-linhay/spaces/20260527-cliproxyapi-upstream-merge/plans/upstream-merge-plan-v01.md` 与 `docs-linhay/dev/20260519-cliproxyapi-upstream-sync-session-distillation.md`；本次仍属于既有 CLIProxyAPI fork 同步流程，不升级 `AGENTS.md`。
