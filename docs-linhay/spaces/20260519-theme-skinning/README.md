...
- 进展补充：2026-06-20 追加完成 `SessionManagementView.tsx` token/shadow cleanup、`CardSections.tsx` typography cleanup 与 `ThemePresetPicker.tsx` quiet shell；这三个文件不再作为当前运行态旧样式高信号入口。下一批运行态旧样式高信号入口更新为 `RateLimitRulesSection.tsx`、`AccountDetailSections.tsx`、`quotaColor.ts`、`UnifiedAccountDetailModal.tsx` 和 `AssetWorkbenchShell.tsx` 等小切片。
