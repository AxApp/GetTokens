...
export default function OpenAICompatibleDetailPanel({
  ...
  const suggestedModels: OpenAICompatibleModelRow[] =
    suggestedModelOptions.models;
  const effectiveVerifyModel =
    draft.verifyModel || suggestedModels[0]?.name || "";
      ...
      <header className="shrink-0 border-b-2 border-[var(--border-color)] px-6 py-4">
        <div className="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
          ...
          <div className="w-full max-w-sm space-y-2 border border-[var(--border-color)] bg-[var(--bg-surface)] px-4 py-3">
            ...
            <div
              className={`text-[length:var(--font-size-ui-sm)] font-black uppercase tracking-tight ${messageTone}`}
            >
      ...
      <div className="min-h-0 flex-1 overflow-auto">
        <AccountDetailBody>
          ...
          <AccountDetailModuleStack layout="cards" cardColumns={1}>
            <AccountDetailSection
              componentName="OpenAICompatibleEndpointSection"
              eyebrow="Endpoint"
              title={t("accounts.openai_provider_name")}
              meta={draft.currentName}
            >
              <div className="grid gap-4 md:grid-cols-2">
                <label className="space-y-2">
                  ...
                  <input
                    ...
                    placeholder={
                      selectedPreset?.baseUrl || "https://api.deepseek.com/v1"
                    }
                  />
                </label>
              ...
              {selectedPreset?.notes ? (
                <AccountDetailNotice tone="neutral" className="mt-4 normal-case">
                  <span>{selectedPreset.notes}</span>
                  {selectedPreset.consoleUrl ? (
                    <a
            ...
            <AccountProxyRouteSection
              ...
              onProxyUrlChange={(nextProxyURL) =>
                onChange({ ...draft, proxyUrl: nextProxyURL })
              }
              ...
              {draft.headersText || headersExpanded ? (
                <>
                  <textarea
                    ...
                    onChange={(event) =>
                      onChange({ ...draft, headersText: event.target.value })
                    }
            ...
            <AccountDetailSection
              ...
              actions={
                <>
                  {remoteModelsState?.status === "success" &&
                  remoteModelsState.models.length > 0 ? (
                    <button
                      onClick={onApplyFetchedModels}
                ...
                {draft.models.map((row, index) => (
                  ...
                  >
                    <input
                      ...
                      placeholder={
                        suggestedModels[0]?.name || "deepseek-v4-flash"
                      }
                    />
                    <input
                      value={row.alias}
                    ...
                    <button
                      onClick={() => {
                        const nextModels = draft.models.filter(
                          (_, itemIndex) => itemIndex !== index,
                        );
                        const nextVerifyModel =
                          draft.verifyModel === row.name
                            ? nextModels[0]?.name || ""
                            : draft.verifyModel;
                        onChange({
                          ...draft,
            ...
            <AccountDetailSection
              componentName="OpenAICompatibleVerifySection"
              muted
              eyebrow="Connection"
              title={t("accounts.openai_provider_test_model")}
            >
              <div className="space-y-4">
                      ...
                      .map((item) => {
                        ...
                        return (
                          <button
                            key={`${item.name}:${item.alias}`}
                            onClick={() =>
                              onChange({ ...draft, verifyModel: modelName })
                            }
                ...
                <div className="flex items-end gap-3">
                  <input
                    ...
                    onChange={(event) =>
                      onChange({ ...draft, verifyModel: event.target.value })
                    }
                    list="openai-compatible-remote-models"
                    className="input-swiss flex-1"
                    placeholder={
                      selectedPreset?.models[0]?.name || "deepseek-v4-flash"
                    }
                  />
                  <button
                    onClick={onVerify}
      ...
      <footer className="flex shrink-0 flex-col gap-3 border-t-2 border-[var(--border-color)] bg-[var(--bg-surface)] px-6 py-4 sm:flex-row sm:items-center sm:justify-between">
        <div className="text-[length:var(--font-size-ui-xs)] font-black uppercase tracking-[0.15em] text-[var(--text-muted)] sm:max-w-[70%]">
          {proxyRouteError ||
            footerMessage ||
            verifyState.message ||
            t("accounts.openai_provider_test_idle")}
        </div>
        <div className="flex items-center gap-2">
          <button
            onClick={() => void onSave()}
            className="btn-swiss bg-[var(--text-primary)] !text-[var(--bg-main)]"
            disabled={saving || Boolean(proxyRouteError)}
          >
