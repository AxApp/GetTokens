...
- 收尾补充：2026-06-20 追加完成 `ClaudeCodeAccountListFeature.tsx` 页面级 quiet shell，范围限定为根背景、route probe 入口按钮和 summary cards；`ClaudeCodeAccountListFeature` 不再作为当前旧样式高信号入口，后续继续优先看 `CardSections.tsx`、`AccountDetailSections.tsx`、Vendor/Status 局部残余等独立切片。
